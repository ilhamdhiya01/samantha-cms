...
async function countPosts(): Promise<number> {
  const res = await apiClient.get("/posts", { params: { pageSize: 1 } });
  return (res.data as { meta?: { total?: number } }).meta?.total ?? 0;
}
async function countProjects(): Promise<number> {
  const res = await apiClient.get("/projects", { params: { pageSize: 1 } });
  return (res.data as { meta?: { total?: number } }).meta?.total ?? 0;
}
...
async function countMedia(): Promise<number> {
  const res = await apiClient.get("/media", { params: { pageSize: 1 } });
  return (res.data as { meta?: { total?: number } }).meta?.total ?? 0;
}
...
export function Dashboard() {
  ...
  const { data: projects } = useQuery({
    queryKey: ["count", "projects"],
    queryFn: countProjects,
  });
  const { data: experiences } = useQuery({
    queryKey: ["count", "experiences"],
    queryFn: countExperiences,
  });
  const { data: media } = useQuery({
    queryKey: ["count", "media"],
    queryFn: countMedia,
  });
