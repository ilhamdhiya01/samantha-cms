...
export function AppLayout() {
  ...
  const mobileItems = navItems.map(({ to, label, end }) => ({ to, label, end }));
    ...
    <div className="min-h-screen flex bg-slate-100">
      ...
      <aside className="hidden md:flex md:w-60 md:flex-col border-r border-slate-200 bg-white">
        <div className="px-5 py-4 border-b border-slate-200">
          <div className="text-sm uppercase tracking-wide text-slate-400">Samantha</div>
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <main id="main" tabIndex={-1} className="flex-1 p-4 sm:p-6 overflow-y-auto">
          <Outlet />
        </main>
      </div>
    </div>
