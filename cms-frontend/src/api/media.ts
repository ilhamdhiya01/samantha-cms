...
export async function listMedia(params: { bucket?: string; page?: number; pageSize?: number } = {}) {
...
export async function uploadMedia(file: File, bucket?: string) {
  ...
  const res = await apiClient.post('/api/media/upload', fd, {
...
export async function deleteMedia(bucket: string, path: string) {
  await apiClient.delete(`/api/media/${bucket}/${path}`);
}
